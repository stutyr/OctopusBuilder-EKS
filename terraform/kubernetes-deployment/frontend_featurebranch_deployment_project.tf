...
resource "octopusdeploy_variable" "frontend_debug_variable_featurebranch" {
...
resource "octopusdeploy_variable" "frontend_debug_evaluated_variable_featurebranch" {
...
locals {
  ...
  dns_branch_name                        = "#{Octopus.Action[Frontend WebApp].Package[${local.frontend_package_name}].PackageVersion | VersionPreRelease | Replace \"\\..*\" \"\" | ToLower}"
...
resource "octopusdeploy_deployment_process" "deploy_frontend_featurebranch" {
  ...
  step {
    condition           = "Success"
    name                = "Display the Main Ingress URL"
    ...
    action {
      ...
      name           = "Display the Main Ingress URL"
      notes          = "Find the URL of the main ingress hosting the backend services. This allows the frontend branch to contact the default backend services."
      run_on_server  = true
      ...
      properties = {
        ...
        "Octopus.Action.Script.ScriptBody" : <<-EOT
          ...
          if [[ "$${DNSNAME}" == "null" ]]
          then
            echo "Deploy the main backend services using the \"Backend Service\" project before deploying a frontend feature branch."
            exit 1
          fi
  ...
  step {
    ...
    action {
      ...
      name           = "Frontend WebApp"
      notes          = "Deploy the web app feature branch to a unique ingress address."
      run_on_server  = true
      worker_pool_id = data.octopusdeploy_worker_pools.ubuntu_worker_pool.worker_pools[0].id
      environments   = [
        data.octopusdeploy_environments.development.environments[0].id,
      ...
      features = [
        ...
        "Octopus.Features.KubernetesConfigMap"
      ]
      package {
      ...
      properties = {
        ...
        "Octopus.Action.KubernetesContainers.IngressName" : local.frontend_featurebranch_ingress_name,
    ...
    action {
      ...
      properties = {
        ...
        "Octopus.Action.Script.ScriptBody" : <<-EOT
          ...
          for i in {1..60}
          do
              DNSNAME=$(kubectl get ingress ${local.frontend_featurebranch_ingress_name} -o json | jq -r '.status.loadBalancer.ingress[0].hostname')
              if [[ "$${DNSNAME}" != "null" ]]
              then
                break
              fi
